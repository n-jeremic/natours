...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // 1) Check if an email and password exist
  if (!email || !password) {
...
exports.protect = catchAsync(async (req, res, next) => {
  // 1) Getting token and check if it's sent with the request
  let token;
  if (
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 3) Update changedPasswordAt property for the user (in document middleware)
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(req.body.currentPassword, user.password))) {
    return next(
      new AppError('Please provide valid password in order to update it!', 401)
    );
  }
